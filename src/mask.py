...
def initMatrixEffect(mask):
    ...
    indexes = np.array(
        [-rd.randint(0, resized_rows - 1) for i in range(resized_cols)])
...
def initStainEffect(mask):
    return {'stains': [getRandomSpawn(mask)]}
...
def updateStainEffect(mask, index, params):
    ...
    for stain in params['stains']:
        ...
        rows_range = range(max(stain['row'] - stain['radius'], 0),
                           min(stain['row'] + stain['radius'], resized_rows))
        cols_range = range(max(stain['col'] - stain['radius'], 0),
                           min(stain['col'] + stain['radius'], resized_cols))
